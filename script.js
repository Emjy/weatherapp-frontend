fetch('https://weatherapp-backend-lyart.vercel.app/weather')
...
function updateDeleteCityEventListener() {
	for (let i = 0; i < document.querySelectorAll('.deleteCity').length; i++) {
		document.querySelectorAll('.deleteCity')[i].addEventListener('click', function () {
			fetch(`http://weatherapp-backend-lyart.vercel.app/weather/${this.id}`, { method: 'DELETE' })
				.then(response => response.json())
				.then(data => {
					if (data.result) {
...
document.querySelector('#addCity').addEventListener('click', function () {
	...
	fetch('http://weatherapp-backend-lyart.vercel.app/weather', {
		method: 'POST',
